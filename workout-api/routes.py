...
def user_data(user: user_dependency, db: db_dependency):
    ...
    return {"User": user, "fullname" : user_db.fullname, 
            "weight" : user_db.weight, "height" : user_db.height}



@router.put("/user_data_change")
def change_user_data(user: user_dependency, db: db_dependency, user_request: ChangeUserDataRequest):
...
def create_goal(user: user_dependency, db: db_dependency, goal: GoalRequestModel):
    ...
    if not goal_type:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Goal type not found")
...
def get_personal_goals(user: user_dependency, db: db_dependency):
    ...
    if not user_goals:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Goals not found on this specific user")
...
def edit_personal_goal(user: user_dependency, goal: GoalRequestModel, goal_id: int, db: db_dependency):
...
def create_schedule(user: user_dependency, db: db_dependency, schedule: ScheduleRequestModel):
...
def delete_personal_schedule(user: user_dependency, schedule_id: int, db: db_dependency):
...
def get_user_history(user: user_dependency, db: db_dependency):
    user_histories = db.query(User_History).filter(
        User_History.user_id == user['id']).all()
