...
class CreateUserRequest(BaseModel):
    username: str = Field(min_length=6, max_length=20, description='Username should be greater than 5, but not higher than 20')
...
class ScheduleRequestModel(BaseModel):
    ...
    end_date: Optional[datetime] = None
    selected_exercises: List[int] = None
    note: Optional[str] = Field(None, max_length=50, description="Note should not exceed 50 chars")
    extended_note: Optional[str] = Field(None, max_length=250, description="Extended note should not exceed 250 chars")
